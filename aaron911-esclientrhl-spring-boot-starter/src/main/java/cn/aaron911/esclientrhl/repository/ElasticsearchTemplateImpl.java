...
//                    .source(BeanTools.objectToMap(tt)));
    ...
    @Override
    public String queryBySQL(String sql, SqlFormatEnum sqlFormat) throws Exception {
        ...
        if(!StringUtils.isEmpty(username)) {
            ...
            HttpRequest httpRequest = HttpUtil.createPost(ipport+"/_sql?format="+sqlFormat.getFormat());
            httpRequest.basicAuth(username, password);
            httpRequest.body("{\"query\":\""+sql+"\"}");
            HttpResponse httpResponse = httpRequest.execute();
            return httpResponse.body();
        }
        //return HttpClientTool.execute(ipport+"/_sql?format="+sqlFormat.getFormat(),"{\"query\":\""+sql+"\"}");
        return HttpUtil.post(ipport+"/_sql?format="+sqlFormat.getFormat(), "{\"query\":\""+sql+"\"}");
    }
    ...
    @Override
    public Map aggs(String metricName, AggsTypeEnum aggsType, QueryBuilder queryBuilder, Class<T> clazz, String bucketName) throws Exception {
        MetaData metaData = IndexTools.getIndexType(clazz);
        String[] indexname = metaData.getSearchIndexNames();
    ...
    @Override
    public Map aggs(String metricName, AggsTypeEnum aggsType, QueryBuilder queryBuilder, Class<T> clazz, String bucketName, String... indexs) throws Exception {
        MetaData metaData = IndexTools.getIndexType(clazz);
...
//        aggregation.order(BucketOrder.key(false));
    ...
    @Override
    public List<Down> aggswith2level(String metricName, AggsTypeEnum aggsType, QueryBuilder queryBuilder, Class<T> clazz, String[] bucketNames) throws Exception {
        MetaData metaData = IndexTools.getIndexType(clazz);
        String[] indexname = metaData.getSearchIndexNames();
    ...
    @Override
    public List<Down> aggswith2level(String metricName, AggsTypeEnum aggsType, QueryBuilder queryBuilder, Class<T> clazz, String[] bucketNames, String... indexs) throws Exception {
        String[] indexname = indexs;
        Field f_metric = clazz.getDeclaredField(metricName.replaceAll(keyword, ""));
        if (bucketNames == null) {
            throw new NullPointerException();
    ...
    public ScrollResponse<T> createScroll(QueryBuilder queryBuilder, Class<T> clazz, Long time, Integer size, String... indexs) throws Exception {
        ...
        ScrollResponse<T> scrollResponse = new ScrollResponse(scrollId, list);
